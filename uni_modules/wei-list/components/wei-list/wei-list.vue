...
<script setup>
  ...
  const { isLoading, loadingText, onLoadmore, isFinished, complete: completeLoading, end: endLoading, reset: resetLoading } = useLoading(props, emit);
  const { onRefresh, complete: completeRefresh, refreshing } = useRefresh(props, { endLoading, isFinished, resetLoading }, emit);
  useProvideList({
    type: 'list',
  })
